  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      const filtered = products.filter(product => 
        ...
        product.device_subcategory?.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
            ...
            <div key={product.id} className="bg-white rounded-xl shadow-sm p-4">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#86868B]">{product.device_category?.name}</p>
                  <p className="text-sm text-[#86868B]">{product.device_subcategory?.name}</p>
                </div>
              </div>
